...
export async function fetchJira (path: string, config: JiraConfig, query: any = {}) {
...
export async function syncJiraActivities (dateStr: string, force: boolean = false) {
  ...
  try {
    const response: any = await fetchJira('search/jql/', config, {
      jql,
      maxResults: 100,
    ...
    const mappedEvents = issues.map((issue: any) => ({
      id: issue.id,
      key: issue.key,
      summary: issue.fields.summary,
      type: issue.fields.issuetype?.name || 'Issue',
