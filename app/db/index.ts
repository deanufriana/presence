...
export async function getDb() {
  ...
  // Initialize the database connection via Tauri's SQL plugin
  ...
  for (const q of queries) {
    try {
      ...
      console.error('Failed to run init query:', q, e)
    }
  }
  ...
  _db = drizzle(
    async (sql, params, method) => {
      try {
        ...
        console.error('Error from sqlite proxy:', message || 'Unknown error')
        return { rows: [] }
      }
    },
    { schema },
