...
export const useYearlyStore = defineStore('yearly', () => {
  ...
  async function fetchYearlyData () {
    try {
      const res: any = await $fetch("/api/report/yearly" as any, {
  ...
  async function generateAiSummary () {
    ...
    try {
      const res: any = await $fetch("/api/report/yearly/summary" as any, {
        method: "POST",
  ...
  watchDebounced(yearlyRows, async (newRows) => {
    if (newRows.length >= 0) {
      await $fetch("/api/report/yearly" as any, {
        method: "POST",
